...
export default class ASTRewriter {
    ...
    if (INCLUDE_AST_WALKER_TRAVERSE_THE_UNIVERSE) {
      ...
      traverse(ast, node => {
        ...
        if (
          ...
          // @ts-ignore
          this.parentNode.parentNode.parentNode === "ObjectExpression" &&
          // @ts-ignore
          this.parentNode.parentNode.parentNode.parentNode ===
            "NewExpression" &&
          // @ts-ignore
          this.parentNode.parentNode.parentNode.parentNode.callee.name ===
            "Proxy"
        ) {
          // @ts-ignore
  ...
  rewriteScript(script: string, isModule: boolean): string {
    const [ast, windowProxyConcealmentAst] = this.parseAst(script, isModule);
    const rewrittenAst = this.rewriteFromAst(ast, windowProxyConcealmentAst);
