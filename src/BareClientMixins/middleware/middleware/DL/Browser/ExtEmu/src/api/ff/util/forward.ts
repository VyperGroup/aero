...
function fwdListener(api: object, eventName: string) {
	...
	api[event].addListener = async function () {
		await sendBrowserData.call(
			`EXT_EVENT_${getEventName(eventName)}_LISTENERS`,
			...arguments,
		);
	};
	...
	api[event].hasListener = async () =>
		await getBrowserData(`EXT_EVENT_${getEventName(eventName)}`);
}
